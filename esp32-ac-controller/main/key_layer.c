                                      ...
                                      uint8_t *shared_secret, size_t *shared_len)
{
    ...
    if (ps != PSA_SUCCESS) { ESP_LOGE(TAG_KEY, "Export priv fail: %d", (int)ps); return false; }
    ...
    if (ps != PSA_SUCCESS) { ESP_LOGE(TAG_KEY, "Import priv fail: %d", (int)ps); return false; }

    *shared_len = 0;
    ps = psa_raw_key_agreement(PSA_ALG_ECDH, ble_key,
                                peer_pubkey, peer_pub_len,
                                shared_secret, 32, shared_len);
    psa_destroy_key(ble_key);
    return (ps == PSA_SUCCESS);
}
