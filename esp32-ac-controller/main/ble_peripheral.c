...
/* 延迟通知队列: 防止 GATT 回调内重入 NimBLE */
#define NOTIFY_QUEUE_MAX 4
static uint8_t s_notify_buf[NOTIFY_QUEUE_MAX][512];
static uint16_t s_notify_len[NOTIFY_QUEUE_MAX];
static int s_notify_head = 0, s_notify_tail = 0;
...
static void deferred_notify_cb(struct ble_npl_event *ev)
{
    ...
    while (s_notify_head != s_notify_tail) {
        ...
        ESP_LOGI("BLE", "def-notify sent: len=%u rc=%d q[%d/%d]", len, rc, s_notify_head, s_notify_tail);
...
void ble_notify_device_event(const uint8_t *data, uint16_t len)
{
    ...
    if (next == s_notify_head) { ESP_LOGW("BLE", "def-notify queue full! drop len=%d", (int)len); return; }
...
static void ble_on_sync(void)
{
    ...
    ble_npl_callout_init(&s_notify_callout, nimble_port_get_dflt_eventq(), deferred_notify_cb, NULL);
...
void ble_peripheral_init(void)
{
    ...
    snprintf(s_adv_name, sizeof(s_adv_name), "YJ-AC-%02X%02X",
