...
bool pairing_load_nvs(void)
{
    ...
    /* 检查配网配置版本号: 旧版本作废, 回退默认值 */
    uint8_t cfg_ver = 0;
    nvs_get_u8(handle, PAIR_NVS_KEY_CFG_VER, &cfg_ver);
    if (cfg_ver < PAIR_NVS_CONFIG_VERSION) {
        ESP_LOGI(TAG_PAIR, "NVS config v%d < current v%d, using defaults",
                 cfg_ver, PAIR_NVS_CONFIG_VERSION);
        nvs_close(handle);
        return false;
    }
    ...
    if (err != ESP_OK) {
        uint8_t mac[6];
        esp_read_mac(mac, ESP_MAC_WIFI_STA);
        snprintf(g_pairing_config.device_id, sizeof(g_pairing_config.device_id),
                 "esp32-%02X%02X%02X%02X%02X%02X",
                 mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    }
...
static bool pairing_save_nvs(const pairing_config_t *cfg)
{
    ...
    nvs_set_u8(handle,  PAIR_NVS_KEY_CFG_VER, PAIR_NVS_CONFIG_VERSION);
...
{
    ESP_LOGW(TAG_PAIR, "!!! 恢复出厂: 擦除 NVS 配网数据 !!!");
    ...
    if (err == ESP_OK) {
        ...
        nvs_commit(handle);
        nvs_close(handle);
        ESP_LOGI(TAG_PAIR, "NVS 配网数据已擦除, 3 秒后重启");
    } else {
        ESP_LOGE(TAG_PAIR, "NVS 打开失败: %d", err);
    }
...
static void pairing_fill_defaults(pairing_config_t *cfg)
{
    ...
    uint8_t mac_d[6];
    esp_read_mac(mac_d, ESP_MAC_WIFI_STA);
    snprintf(cfg->device_id, sizeof(cfg->device_id),
             "esp32-%02X%02X%02X%02X%02X%02X",
