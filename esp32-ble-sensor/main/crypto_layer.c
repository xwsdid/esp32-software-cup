...
bool crypto_decrypt_fallback(const uint8_t *ct, size_t ct_len,
    ...
    if (crypto_aes_decrypt(ct, ct_len, iv, iv_len, tag, tag_len, pt, pt_len))
        return true;
    /* 失败: 回退静态密钥 */
    ESP_LOGW(TAG_CRYPTO, "当前key解密失败, 回退静态key (epoch=外部)");
    if (g_sm4_mode) {
        if (sm4_gcm_decrypt(g_static_sm4_key, ct, ct_len, iv, tag, pt)) {
            *pt_len = ct_len;
            ESP_LOGW(TAG_CRYPTO, "keySource=static (fallback ok, 请大禹重新ECDH)");
            return true;
        }
    } else {
        /* AES回退需要PSA旧密钥, 这里先只支持SM4回退 */
    }
    return false;
}
...
bool sm4_gcm_decrypt(const uint8_t *key,
    ...
    if (memcmp(expected_tag, tag, 16) != 0) return false;
